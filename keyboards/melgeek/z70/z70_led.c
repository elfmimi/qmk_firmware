...
#define write_reg_(adr, dat) if(!write_reg__(adr, dat)) { goto error; }
bool write_reg__(uint8_t adr, uint8_t dat) {
    tx_buf[0] = adr;
    tx_buf[1] = dat;
    msg_t msg = i2cMasterTransmitTimeout(&I2CD0, LED_DRIVER_ADDR >> 1, tx_buf, 2, NULL, 0, MS2ST(5));
    return msg == MSG_OK;
}
...
void write_reg_buf(uint8_t adr, int len) {
    tx_buf[0] = adr;
    msg_t msg = i2cMasterTransmitTimeout(&I2CD0, LED_DRIVER_ADDR >> 1, tx_buf, len + 1, NULL, 0, MS2ST(10));
    (void)msg;
}
...
uint8_t read_reg(uint8_t adr) {
    uint8_t txrxbuf[1];
    txrxbuf[0] = adr;
    msg_t msg = i2cMasterTransmitTimeout(&I2CD0, LED_DRIVER_ADDR >> 1, txrxbuf, 1, txrxbuf, 1, MS2ST(5));
    (void)msg;
    return txrxbuf[0];
}

bool SET_ALL_PWM(uint8_t pwm) {
    write_reg_(0xFE, 0xC5);
    write_reg_(0xFD, 0x00);
    for (int i = 0x00; i<=0xB3; i++) {
        write_reg_(i, pwm);
    }
    write_reg_(0xFE, 0xC5);
    write_reg_(0xFD, 0x01);
