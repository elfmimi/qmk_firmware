...
#undef printf
// #define printf(fmt) fctprintf(sendchar_serial, NULL, fmt, NULL)
// #define printf(fmt, ...) fctprintf(sendchar_serial, NULL, fmt, __VA_ARGS__)
#define printf(...) fctprintf(sendchar_serial, NULL, __VA_ARGS__)
...
void sendchar_serial(void *p, char c) {
    (void)p;
    sdWrite(&SD0, (unsigned char *)&c, 1U);
...
void sendchar_serial(char c, void *p) {
    (void)p;
    sdWrite(&SD0, (unsigned char *)&c, 1U);
#ifdef CONSOLE_ENABLE
    sendchar((uint8_t)c);
